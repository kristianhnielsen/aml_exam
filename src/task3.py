...
def _(
    ...
    with mlflow.start_run(
        ...
        notify_completion(f"FFNN Accuracy: {test_accuracy:.4f}")
    return
...
def _(
    ...
    with mlflow.start_run(run_name="FFNN on Autoencoder Features - run 1"):
        ...
        notify_completion(
            f"FFNN on AE Accuracy: {test_accuracy_enc:.4f} and ROC AUC: {_roc_auc_score_enc:.4f}"
        )
    return
...
def _(
    ...
    # 1. Define your maximum subset size
    MAX_SAMPLES = 1000

    # 2. Subsample X_train and y_train if they are too large
...
def _(
    ...
    with mlflow.start_run(run_name="TabNet_Baseline"):
        _tabnet_model.fit(
            ...
            batch_size=1024,
            virtual_batch_size=128,  # Size of mini-batches for Batch Normalization
            num_workers=0,
            drop_last=False,
        )
...
def _(
    ...
    # 1. TUNED PARAMETERS for Telco (Small-to-medium dataset)
    # n_d/n_a: smaller (8-16) is often better for 7k rows to prevent overfitting
    # n_steps: 3-5 steps allow for enough decision complexity
    _params = {
        "n_d": 16, 
        "n_a": 16,
        "n_steps": 5,
    ...
    with mlflow.start_run(run_name="TabNet_Tuned_Balanced"):
        ...
        # weights=1 calculates class weights automatically to balance Churn (Yes/No)
        _tabnet_tuned.fit(
            X_train=_X_train_np, y_train=_y_train_np,
            ...
            weights=1, # Very important for Churn prediction!
            drop_last=False
        )
        ...
        mlflow.log_metrics({"best_val_auc": max(_tabnet_tuned.history['valid_auc'])})
        notify_completion(f"TabNet Tuned AUC: {_auc_score:.4f} and Accuracy: {_accuracy:.4f}")
        print("Feature importances from tuned TabNet:", _tabnet_tuned.feature_importances_)
    return
