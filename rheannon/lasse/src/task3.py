...
def _(
    ...
    with mlflow.start_run(run_name="Feed Forward Neural Network - run 3"):
        history = ffnn_model.fit(
            ...
            epochs=100,
            batch_size=32,
            callbacks=[early_stopping, lr_scheduler, checkpoint], # saves the best model
        )
...
def _(
    ...
    autoencoder.compile(optimizer="adamw", loss="mse") # had to change this from adamW the second time??
    ...
    with mlflow.start_run(run_name="Autoencoder - run 1"):
        history_ae = autoencoder.fit(
            ...
            epochs=100,
            batch_size=32,
            callbacks=[early_stopping_ae, lr_scheduler_ae, checkpoint_ae], # to save it
        )
...
def _(
    ...
    with mlflow.start_run(run_name="FFNN on Autoencoder Features - run 1"):
        history_enc = ffnn_encoded_model.fit(
            ...
            epochs=100,
            batch_size=32,
            callbacks=[early_stopping_enc, lr_scheduler_enc, checkpoint_enc],
        )
...
def _(
    ...
    with mlflow.start_run(run_name="XGBoost on Autoencoder Features - run 1"):
        ...
        # save model
        # xgb_model.save_model("xgb_encoded_model.json")
...
# need for shap
@app.cell
def _(X_train, X_test, y_train, y_test, preprocessor, np, joblib):
